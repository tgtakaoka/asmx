...
const char tfrRegs[] = "D X Y U S PC    A B CCDP";
	...
	switch(token)
	{
			...
			if (token == '-')
			{
				if (*linePtr == '-')
				{
					...
					if (reg < 0)
						...
						XInstr2(idxOp, reg * 0x20 + 0x83 + indirect);  // ,--X
				}
				else
				{
					GetWord(word);
					...
					if (reg < 0)
						...
						XInstr2(idxOp, reg * 0x20 + 0x82 + indirect);  // ,-X
				}
			}
			else
			{
				...
				else
				{
					...
					switch(token)
					{
						case ']':
							linePtr = oldLine;
						case 0:
							XInstr2(idxOp, reg * 0x20 + 0x84 + indirect);  // ,X
							break;
						case '+':
							if (*linePtr == '+')
							{
								linePtr++;
								XInstr2(idxOp, reg * 0x20 + 0x81 + indirect);  // ,X++
							}
							else
								XInstr2(idxOp, reg * 0x20 + 0x80 + indirect);  // ,X+
							break;
						default:
							BadMode();
							break;
		...
		case 'D':
			...
			else
			{
				switch(token)
				{
					case 'A':   XInstr2(idxOp, reg * 0x20 + 0x86 + indirect); break;		// A,X
					case 'B':   XInstr2(idxOp, reg * 0x20 + 0x85 + indirect); break;		// B,X
					case 'D':   XInstr2(idxOp, reg * 0x20 + 0x8B + indirect); break;		// D,X
					default:	BadMode();
				}
			}
			break;
			...
			switch(token)
			{
				...
				case 0:		// value
					if (!indirect && (dirOp >= 0 || extOp >= 0))
					{
						...
						XInstr4W(idxOp, 0x8F + indirect, val);  // $xxxx
					break;
				case ',':   // value,
					GetWord(word);
					reg = FindReg(word, idxRegs);
					if (reg < 0)
					{
						if (strcmp(word,"PC") == 0 || strcmp(word,"PCR") == 0)
						{
							...
							if ((force != '>' && evalKnown && -128 <= val && val <= 127) || force == '<')
								XInstr3(idxOp, 0x8C + indirect, val);		// nn,PCR
							else
								XInstr4W(idxOp, 0x8D + indirect, val - 1); // nnnn,PCR
						}
						else
							BadMode();
					}
					else
					{
						if (force != '>' && evalKnown && !indirect && -16 <= val && val <= 15)
							XInstr2(idxOp, reg * 0x20 + (val & 0x1F));  // n,X
						else if (evalKnown && -128 <= val && val <= 127)
							XInstr3(idxOp, reg * 0x20 + 0x88 + indirect, val);  // nn,X
						else
							XInstr4W(idxOp, reg * 0x20 + 0x89 + indirect, val);	// nnnn,X
					}
					break;
